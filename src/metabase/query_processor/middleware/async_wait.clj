...
(defn- runnable ^Runnable [qp query xform respond raise canceled-chan]
  (bound-fn []
...
(defn- run-in-thread-pool [qp {database-id :database, :as query} xform respond raise canceled-chan]
  {:pre [(integer? database-id)]}
  (try
    (let [pool  (db-thread-pool database-id)
          futur (.submit pool (runnable qp query xform respond raise canceled-chan))]
      (a/go
...
(defn wait-for-turn
  ...
  (fn [query xform respond raise canceled-chan]
    ...
    (if (or *already-in-thread-pool?* *disable-async-wait*)
      (qp query xform respond raise canceled-chan)
      (run-in-thread-pool qp query xform respond raise canceled-chan))))
