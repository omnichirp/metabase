  ...
  (let [row-count (atom 0)]
    (fn
      ...
      ([result results-metadata row]
       (let [result'       (xf result results-metadata row)
...
(defn limit
  ...
  (fn [query xform respond raise canceled-chan]
    (let [max-rows (or (mbql.u/query->max-rows-limit query)
      ...
      (try
        (qp (add-limit max-rows query) (comp (partial limit-xform max-rows) xform) respond raise canceled-chan)
        (catch Throwable e
          (raise e))))))
